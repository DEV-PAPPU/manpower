<template>
    <div class="mt-4">

         <!-- Modal -->
        <div class="modal fade" id="modelimage" tabindex="-1" role="dialog"
            aria-labelledby="modelimageTitle" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="modelimageTitle">{{modalImage.title}}</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body">
                        <div>
                            <img :src="modalImage.image" width="450px" height="auto" alt="" srcset="">
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-primary" data-dismiss="modal">Close</button>
                    </div>
                </div>
            </div>
        </div>
        <div class="row">
            <div class="col-xl-4">
                <!-- Profile picture card-->
                <div class="card mb-4 mb-xl-0">
                    <div class="text-white card-header">Profile Picture</div>
                    <div class="card-body text-center">
                        <!-- Profile picture image-->
                        <img class="profile_image" :src="passenger.passenger_photo" alt="">

                        <div class="card-body p-1-9 p-xl-5">
                            <div class="mb-2">
                                <h3 class="agent_info h4 mb-0"> Name: {{passenger.passenger_name}}</h3>
                            </div>

                            <div class="mt-3">
                                <p class="agent_info"><i
                                        class="fas fa-mobile-alt display-25 me-3 text-secondary"></i>{{passenger.passenger_phone}}
                                </p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-xl-8">


                  <!-- Agent info card-->
                <div class="card mb-4">
                    <div class="text-white card-header d-flex flex-row align-items-center justify-content-between">
                        <span>Passenger Info</span>
                        <button @click="togglePassengerinfo" class="btn btn-light btn-sm">Show</button>
                    </div>
                    <div v-if="showPassengerInfo" class="card-body">

                        <div class="database__table">
                    <table class="table table-hover table-bordered">
                        <thead>
                            <tr>
                                <th style="width:100px">Passport No</th>
                                <th style="width:100px">Old Passport No</th>
                                <th style="width:100px">Passport Exp D</th>
                                <th style="width:100px">Father Name</th>
                                <th style="width:100px">Gurdian's No</th>
                                <!-- <th style="width:100px">District</th> -->
                                <!-- <th style="width:90px">Passport Source</th> -->
                                <!-- <th style="width:100px">Agent Name</th> -->
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td>{{passenger.passport_no}}</td>
                                <td>{{passenger.old_passport_no}}</td>
                                <td>{{passenger.passport_expire_date}}</td>
                                <td>{{passenger.passenger_father_name}}</td>
                                <td>{{passenger.passenger_gurdian_no}}</td>
                                <!-- <td>{{item.district_name}}</td> -->
                                <!-- <td>{{item.passport_source}}</td> -->
                                <!-- <td>{{item.agent_name}}</td> -->
                            </tr>

                        </tbody>
                    </table>
                </div>

                    </div>
                </div>


    

                <!-- Passenger Images-->
                <div class="card mb-4">
                    <div class="text-white card-header d-flex flex-row align-items-center justify-content-between">
                        <span>Passenger Images</span>
                        <button @click="toggleImage" class="btn btn-light btn-sm">Show</button>
                    </div>
                    <div v-if="showPassengerImage" class="card-body">
                        <div class="mt-4 images">
                            <div v-if="images.length" class="d-flex gap-3 flex-wrap">
                                <table class="table table-hover table-bordered dbtable">
                        <thead>
                            <tr>
                                <th>Title</th>
                                <th>Image</th>
                                <th>Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr v-for="item in images" :key="item.id">
                                <td>{{item.title}}</td>
                                <td><img :src="item.image" class="img-fluid border" style="width:100px" alt="" srcset=""> </td>
                                <td>
                                    
                                    <button @click="showImage(item)" data-toggle="modal" data-target="#modelimage"
                                     class="btn btn-success " >View Image </button>
                                    <!-- <button @click="deleteImage(item)" class="mx-3 btn btn-success " >Delete Image </button> -->
                                    <!-- <button @click="downloadImage(item.image)" class="btn btn-success " >Download Image<i class="fas fa-arrow-circle-right px-2"></i></button> -->
                                </td>
                            </tr>

                        </tbody>
                    </table>
                            </div>
                            <p v-else class="p-6">No Images</p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</template>

<script>
// import 'jquery/dist/jquery.min.js';
// //Datatable Modules
// import "datatables.net-dt/js/dataTables.dataTables"
// import "datatables.net-dt/css/jquery.dataTables.min.css"
import $ from 'jquery'; 
import axios from 'axios';
    export default {
        data: () =>{
            return {
               passenger:'',
               images: '',
               showPassengerInfo: true,
               showPassengerImage: false,
               modalImage: ''
            }
        },

        methods:{

            togglePassengerinfo(){
                 
                  this.showPassengerImage = false;
                  this.showPassengerInfo = !this.showPassengerInfo
            },

            toggleImage(){
                 
                    this.showPassengerInfo = false;
                    this.showPassengerImage = !this.showPassengerImage;

            },

            showImage(image){
            this.modalImage = image;
        },
        },

        mounted() {

            let id = this.$route.params.id;
           
            axios.get(`edit-passenger/${id}`).then(res =>{
                this.passenger = res.data.passenger;
                console.log(res.data.passenger)
            });


            axios.get(`passenger-image/${id}`).then((res)=>{
                 this.images = res.data;
                // setTimeout(() => {
                //     $(".dbtable").DataTable({
                //         lengthMenu: [
                //         [5,10, 25, 50, -1],
                //         [5,10, 25, 50, "All"],
                //         ],
                //         pageLength: 5,
                //     });
                //     });
              })



        }
    }
</script>


<style scoped>
body{margin-top:20px;
background-color:#f2f6fc;
color:#69707a;
}
.img-account-profile {
    height: 10rem;
}

.card {
    box-shadow: 0 0.15rem 1.75rem 0 rgb(33 40 50 / 15%);
}
.card .card-header {
    font-weight: 500;
}
.card-header:first-child {
    border-radius: 0.35rem 0.35rem 0 0;
}
.card-header {
    padding: 1rem 1.35rem;
    margin-bottom: 0;
    background-color: rgba(33, 40, 50, 0.03);
    border-bottom: 1px solid rgba(33, 40, 50, 0.125);
}
.form-control, .dataTable-input {
    display: block;
    width: 100%;
    padding: 0.875rem 1.125rem;
    font-size: 0.875rem;
    font-weight: 400;
    line-height: 1;
    color: #69707a;
    background-color: #fff;
    background-clip: padding-box;
    border: 1px solid #c5ccd6;
    -webkit-appearance: none;
    -moz-appearance: none;
    appearance: none;
    border-radius: 0.35rem;
    transition: border-color 0.15s ease-in-out, box-shadow 0.15s ease-in-out;
}

.nav-borders .nav-link.active {
    color: #0061f2;
    border-bottom-color: #0061f2;
}
.nav-borders .nav-link {
    color: #69707a;
    border-bottom-width: 0.125rem;
    border-bottom-style: solid;
    border-bottom-color: transparent;
    padding-top: 0.5rem;
    padding-bottom: 0.5rem;
    padding-left: 0;
    padding-right: 0;
    margin-left: 1rem;
    margin-right: 1rem;
}

.profile_image{
    width: 300px;
    height: auto;
    object-fit: cover;
    border-radius: 10px;
    padding: 2px;
    border: dotted black 2px;
}

.agent_info{
    text-align: left;
}

.agent_images{
    width: 200px;
    height: 130px;
    object-fit: cover;
    border-radius: 10px;
}

table.dataTable thead th, table.dataTable thead td {
    font-size: 12px;
    font-weight: 500;
    color: rgb(43, 43, 43);
}

.table td, .table th {
    padding: 0.75rem;
    vertical-align: top;
    border-top: 1px solid #e3e6f0;
    font-size: 14;
}

.agent_ref{
    display: flex;
}
</style>